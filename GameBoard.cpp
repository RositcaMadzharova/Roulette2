...
void GameBoard::DrawWiningNumber(SpinScreen* spin)
{
		SDL_Rect dest{600, 400 , 200,200 };
		SDL_Rect source = DisplayWiningNumber(spin->GetWinningNumber());
		int blend = 200;

		while(dest.w < SCREEN_W -200 && dest.h < SCREEN_H - 200)
		{
		...
		SDL_RenderCopyEx(LWindow::gRenderer, background->getTexture(),&source, &dest, 0 , NULL,SDL_FLIP_NONE);
			background->setAlpha(blend);
...
void GameBoard::DisplayStatistics(Credits* credits, int lastWinningNumber)
{
	...
	if (lastWinningNumber != -1)
		Text textWin(SCREEN_W * 3 / 4, 40, 30, 35, 15,
				lastWinningNumber,
				{ 200, 200, 200, 255 });
	else
...
SDL_Rect GameBoard::DisplayWiningNumber(int lastWinningNumber)
{
	vector<SDL_Rect> source;
	source.push_back({72,376,82,82});
	int w = 82, h = 82 ;
	SDL_Rect first { 153 , 458 , w , h };
	for(int i = 0 ; i < 12 ; i ++)
	{
		for(int j = 2 ; j >= 0 ; j--)
			{
	...
	return source[lastWinningNumber];
}
